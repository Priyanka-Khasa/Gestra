...
function requireMain() {
  ...
  const { keyboard, Key, mouse, screen } = require$$3;
  ...
  async function proxyAssistantRequest(payload) {
    ...
    if (!apiKey) {
      throw new Error(
        `Missing ${provider === "openrouter" ? "OpenRouter" : provider === "xai" ? "xAI" : "Gemini"} API key.`
      );
    }
    ...
    try {
      if (provider === "openrouter") {
        const url2 = buildOpenRouterUrl(baseUrl);
        ...
        if (!response2.ok) {
          throw new Error(
            `OpenRouter request failed (${response2.status}): ${JSON.stringify((data2 == null ? void 0 : data2.error) || data2 || {})}`
          );
        }
        return ((_c = (_b = (_a = data2 == null ? void 0 : data2.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) || "No response from OpenRouter.";
      }
      if (provider === "xai") {
        ...
        const data2 = await response2.json().catch(() => null);
        if (!response2.ok) {
          throw new Error(`xAI request failed (${response2.status}): ${JSON.stringify((data2 == null ? void 0 : data2.error) || data2 || {})}`);
        }
        return ((_f = (_e = (_d = data2 == null ? void 0 : data2.choices) == null ? void 0 : _d[0]) == null ? void 0 : _e.message) == null ? void 0 : _f.content) || "No response from xAI.";
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(`Gemini request failed (${response.status}): ${JSON.stringify((data == null ? void 0 : data.error) || data || {})}`);
      }
      return ((_k = (_j = (_i = (_h = (_g = data == null ? void 0 : data.candidates) == null ? void 0 : _g[0]) == null ? void 0 : _h.content) == null ? void 0 : _i.parts) == null ? void 0 : _j[0]) == null ? void 0 : _k.text) || "No response from Gemini.";
    } catch (error) {
  ...
  async function executeOsActionWithFallback(actionRaw, options = null) {
    ...
    try {
      ...
      return {
        ok: false,
        message: `Action failed: ${String((error == null ? void 0 : error.message) || error)}`
      };
    }
  }
  async function executeDesktopLaunch(targetRaw) {
    ...
    try {
      ...
      return {
        ok: false,
        message: `Failed to open ${target}: ${String((error == null ? void 0 : error.message) || error)}`
      };
    }
  }
  async function executeVoiceCommand(payload) {
    const type = String((payload == null ? void 0 : payload.type) || "").trim().toLowerCase();
    if (type === "open-app") {
      return executeDesktopLaunch(payload == null ? void 0 : payload.target);
    }
    if (type === "os-action") {
      const action = String((payload == null ? void 0 : payload.target) || "").trim();
      await yieldFocusToDesktop({ hideWindow: true });
      return executeOsActionWithFallback(action, (payload == null ? void 0 : payload.options) ?? null);
    }
    if (type === "window-action") {
      const action = String((payload == null ? void 0 : payload.target) || "").trim().toLowerCase();
      if (action === "show") {
  ...
  async function recognizeNativeSpeech(payload = {}) {
    ...
    const timeoutSeconds = Math.max(3, Math.min(15, Math.round(Number(payload == null ? void 0 : payload.timeoutSeconds) || 8)));
    ...
    return await new Promise((resolve) => {
      ...
      child.on("error", (error) => {
        finish({ ok: false, reason: "spawn-failed", message: String((error == null ? void 0 : error.message) || error) });
      });
      child.on("exit", () => {
  ...
  async function ensurePythonBackend() {
    ...
    return {
      ...
      launchCommand: pythonLaunchCommand ? `${pythonLaunchCommand.cmd} ${pythonLaunchCommand.args.join(" ")}` : null,
      entry: PYTHON_ENTRY,
